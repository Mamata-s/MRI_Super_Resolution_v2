...
from densenet import  SRDenseNetUpscale
from dataset import RdnSampler,MRIDatasetUpscale
...
if __name__ == '__main__':
    ...
    model = SRDenseNetUpscale(num_channels=1, growth_rate=args.growth_rate, num_blocks = args.num_blocks, num_layers=args.num_layers).to(device)
    # print(model)
    ...
    train_datasets = MRIDatasetUpscale(train_img_dir, train_label_dir, dir_dict = dir_traindict,test=False)
    val_datasets = MRIDatasetUpscale(val_img_dir, val_label_dir, dir_dict = val_dir_traindict,test=True)
    ...
    for epoch in range(args.num_epochs):
        ...
        with tqdm(total=(len(train_datasets) - len(train_datasets) % args.batch_size), ncols=80) as t:
            ...
            for idx, (images, labels) in enumerate(train_dataloader):
                ...
                t.update(len(images))
        if k == 100:
            torch.save(model.state_dict(), os.path.join(args.outputs_dir, 'epoch_{}_f_2.pth'.format(epoch)))
            k=0
        k+=1
    ...
    path = f'dense_upscale_e'+str(args.num_epochs)+'_f2'+'_final.pth'
