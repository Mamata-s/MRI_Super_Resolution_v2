...
from dataset import MRIDataset, RdnSampler, MRIDatasetPatch
...
if __name__ == '__main__':
    ...
    train_img_dir = 'dataset/patch-data/'+ str(str_train_2)
    val_img_dir = 'dataset/patch-data/'+str(str_val_2)
  

    train_label_dir = 'dataset/patch-data/factor_2/label/train'
    val_label_dir = 'dataset/patch-data/factor_2/label/val'
    ...
    train_datasets = MRIDatasetPatch(train_img_dir, train_label_dir, dir_dict = dir_traindict,test=False)
    val_datasets = MRIDatasetPatch(val_img_dir, val_label_dir, dir_dict = val_dir_traindict,test=True)
    ...
    eval_dataloader = torch.utils.data.DataLoader(val_datasets, batch_size = val_batch_size,shuffle=True,
        num_workers=1,pin_memory=False,drop_last=False)
    ...
    print('train loader image shape',images.shape)
    ...
    path = f'dense_patch_e'+str(args.num_epochs)+'_f'+str(args.factor)+'_final.pth'
    ...
    with open("losses_patch", "wb") as fp:   #Pickling
