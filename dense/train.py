...
if __name__ == '__main__':
    ...
    for epoch in range(args.num_epochs):
        ...
        with tqdm(total=(len(train_datasets) - len(train_datasets) % args.batch_size), ncols=80) as t:
            ...
            for idx, (images, labels,parsers) in enumerate(train_dataloader):
                ...
                t.update(len(images))
        if k == 100:
            torch.save(model.state_dict(), os.path.join(args.outputs_dir, 'epoch_{}_f_{}.pth'.format(epoch,args.factor)))
            k=0
        k+=1
    ...
    if args.bicubic:
        print('reached here')
        path = f'dense_bicubic_e'+str(args.num_epochs)+'_f'+str(args.factor)+'_final.pth'
    else:
        path = f'dense_e'+str(args.num_epochs)+'_f'+str(args.factor)+'_final.pth'
